...
-- Read the RXZ config, but fall back to an existing Rainy one so the
-- rename does not wipe saved settings. Writes always go to the new file.
_GACC.CFG_FILE = "RXZHub_PC.json"
if isfile and not isfile(_GACC.CFG_FILE) then
    for _,legacy in ipairs({"RitualHub_PC.json","RainyHub_PC.json"}) do
        if isfile(legacy) then _GACC.CFG_FILE = legacy break end
    end
end
...
local THEME_DEFS = {
    RAIN = {accent=Color3.fromRGB(170,110,255), accentDark=Color3.fromRGB(96,45,175), accentBg=Color3.fromRGB(26,10,48), accentHover=Color3.fromRGB(60,26,105), accentRowHover=Color3.fromRGB(26,10,43), bgAsset=""},
}
...
if introSoundEnabled then
    task.spawn(function()
        ...
        local numeFisier = "rxzhub_intro.mp3"
...
if false then
    task.spawn(function()
        ...
        introGui.Name = "RXZHubIntroTitle"
        ...
        pokerLbl.Text = 'RXZ <font color="rgb(190,150,255)">HUB</font>'
...
local function applyHeadless(char,enabled)
    ...
    if enabled then
        ...
        local old=head:FindFirstChild("RXZHeadlessMesh"); if old then old:Destroy() end
        local mesh=Instance.new("SpecialMesh",head); mesh.Name="RXZHeadlessMesh"; mesh.MeshType=Enum.MeshType.FileMesh; mesh.MeshId=HEADLESS_MESH_ID; mesh.Scale=Vector3.new(.001,.001,.001)
    else
        local mesh=head:FindFirstChild("RXZHeadlessMesh"); if mesh then mesh:Destroy() end
...
local function applyKorblox(char,enabled)
    ...
    if hum.RigType==Enum.HumanoidRigType.R6 then
        ...
        if enabled then
            ...
            leg.Color=Color3.fromRGB(64,64,64); local old=leg:FindFirstChild("RXZKorbloxMesh"); if old then old:Destroy() end
            local mesh=Instance.new("SpecialMesh",leg); mesh.Name="RXZKorbloxMesh"; mesh.MeshType=Enum.MeshType.FileMesh; mesh.MeshId=KORBLOX_MESH_ID; mesh.TextureId=KORBLOX_TEXTURE_ID
        else
            local mesh=leg:FindFirstChild("RXZKorbloxMesh"); if mesh then mesh:Destroy() end
        ...
        if enabled then
            ...
            local old=char:FindFirstChild("RXZKorbloxLeg"); if old then old:Destroy() end
            local leg=Instance.new("Part",char); leg.Name="RXZKorbloxLeg"; leg.Size=Vector3.new(1,2,1); leg.Anchored=false; leg.CanCollide=false; leg.Massless=true; leg.Color=Color3.fromRGB(64,64,64)
            ...
            local weld=Instance.new("Weld",leg); weld.Name="RXZKorbloxWeld"; weld.Part0=upper; weld.Part1=leg; weld.C0=CFrame.new(0,-.8,0)
        else
            ...
            local leg=char:FindFirstChild("RXZKorbloxLeg"); if leg then leg:Destroy() end; state.r15Transparency=nil
        end
    end
end
...
function instantReset()
    ...
    RunService:BindToRenderStep("RXZCamLock", Enum.RenderPriority.Camera.Value + 1, function()
        cam.CameraType = Enum.CameraType.Scriptable
        cam.CFrame = lockedCamCFrame
    ...
    respawnConn = LP.CharacterAdded:Connect(function(newChar)
        ...
        pcall(function()
            RunService:UnbindFromRenderStep("RXZCamLock")
        end)
...
-- Dice Bat engine adapter for RXZ Hub.
...
function Overhead.setupHeadIndicator(char)
    ...
    line.Position = UDim2.new(0.20, 0, 0, 22)
...
-- Auto path waypoints (RXZ Hub coords)
...
-- RXZ Hub steal presets. Fixed on purpose - V modes expose no settings.
        ...
        if _GACC.startRXZSemi then
            ...
            task.spawn(function()
                for _=1,40 do
                    ...
                    if _GACC.startRXZSemi then _GACC.startRXZSemi(); return end
                end
            end)
        end
        return
...
stopSemiAutoSteal=function()
    if _GACC.stopRXZSemi then _GACC.stopRXZSemi() end
...
saveConfig=function()
    ...
    if writefile then pcall(function() writefile(_GACC.CFG_FILE,HS:JSONEncode(cfg)) end) end
end
...
local function createStealBar()
    ...
    stealBarFrame.BackgroundColor3=Color3.fromRGB(20,8,40)
    ...
    Instance.new("UICorner",stealBarFrame).CornerRadius=UDim.new(0,7)
    ...
    sbBgGrad.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(84,42,150)),ColorSequenceKeypoint.new(0.55,Color3.fromRGB(40,16,74)),ColorSequenceKeypoint.new(1,Color3.fromRGB(20,6,36))})
    ...
    stealLbl.TextColor3=Color3.fromRGB(232,220,255); stealLbl.Font=Enum.Font.GothamBold; stealLbl.TextSize=10
    ...
    stateLbl.TextColor3=Color3.fromRGB(150,120,200); stateLbl.TextSize=8; stateLbl.Font=Enum.Font.GothamBold
    ...
    perfLbl.TextColor3=Color3.fromRGB(170,140,215); perfLbl.TextSize=8; perfLbl.Font=Enum.Font.GothamMedium
    ...
    track.BackgroundColor3=Color3.fromRGB(60,30,105); track.BackgroundTransparency=.48; track.BorderSizePixel=0; track.ZIndex=21
    ...
    table.insert(_themeExtRefs,{callback=function(thm)
        pcall(function() stealBarFrame.BackgroundColor3=Color3.fromRGB(20,8,40) end)
    ...
    stealLbl.Visible=false; stateLbl.Visible=false; perfLbl.Visible=false
    pctLbl.Visible=false; dot.Visible=false; sideAccent.Visible=false
    ...
    task.spawn(function()
        ...
        while fillLine and fillLine.Parent do
            ...
            stateLbl.TextColor3=ready and Color3.fromRGB(200,170,255) or Color3.fromRGB(150,120,200)
...
local function setupMobileButtons()
    ...
    local function makeButton(name, text, defaultActive, order)
        ...
        -- RXZ Hub press feel: squash to 0.88, spring back with Back easing.
...
;(function()
    ...
    local grad=Instance.new("UIGradient")
    grad.Color=ColorSequence.new({
        ColorSequenceKeypoint.new(0,   Color3.fromRGB(10,2,20)),
        ColorSequenceKeypoint.new(0.35,Color3.fromRGB(22,8,40)),
        ColorSequenceKeypoint.new(0.65,Color3.fromRGB(30,12,52)),
        ColorSequenceKeypoint.new(1,   Color3.fromRGB(14,4,26)),
    })
    ...
    do
        ...
        headerPlate.BackgroundColor3=Color3.fromRGB(22,8,40); headerPlate.BackgroundTransparency=.72; headerPlate.BorderSizePixel=0; headerPlate.ZIndex=2
        ...
        plateGrad.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(46,18,84)),ColorSequenceKeypoint.new(.48,Color3.fromRGB(24,9,44)),ColorSequenceKeypoint.new(1,Color3.fromRGB(14,5,26))})
    ...
    TL.Position=UDim2.new(0,28,0,13); TL.Size=UDim2.new(0,68,0,27); TL.BackgroundTransparency=1
    TL.Text="RXZ"; TL.TextColor3=C.text; TL.TextSize=18; TL.Font=Enum.Font.GothamBold
    ...
    PrivateTL.Text="HUB"; PrivateTL.TextColor3=Color3.fromRGB(214,186,255); PrivateTL.TextSize=18; PrivateTL.Font=Enum.Font.GothamBold
    ...
    privateGrad.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(96,45,175)),ColorSequenceKeypoint.new(.42,Color3.fromRGB(178,124,255)),ColorSequenceKeypoint.new(.58,Color3.fromRGB(245,250,255)),ColorSequenceKeypoint.new(1,Color3.fromRGB(130,80,210))})
    ...
    ML.Text="discord.gg/rxzhub"; ML.TextColor3=C.textDim; ML.TextSize=9; ML.Font=Enum.Font.GothamMedium
    ...
    MiniBtn.BackgroundColor3=Color3.fromRGB(0,0,0); MiniBtn.BackgroundTransparency=0; MiniBtn.BorderSizePixel=0
    MiniBtn.RichText=true; MiniBtn.Text='RXZ <font color="rgb(255,255,255)">HUB</font>'; MiniBtn.TextColor3=Color3.fromRGB(255,255,255); MiniBtn.Font=Enum.Font.GothamBold; MiniBtn.TextSize=12
    MiniBtn.TextStrokeColor3=Color3.fromRGB(255,255,255); MiniBtn.TextStrokeTransparency=0.62
    MiniBtn.ZIndex=20; MiniBtn.Visible=false; MiniBtn.Parent=GuiRefs.hub
    guiCorner(MiniBtn,8); local miniBtnStroke=guiStroke(MiniBtn,Color3.fromRGB(255,255,255),1.2);miniBtnStroke.Transparency=.55
    ...
    MiniBtn.MouseEnter:Connect(function() 
        ...
        tw(miniBtnStroke,{Transparency=.2})
    end)
    MiniBtn.MouseLeave:Connect(function() 
        tw(MiniBtn,{BackgroundTransparency=0})
        tw(miniBtnStroke,{Transparency=.55})
    end)
    local function showGui()
        local home=(_GACC.getUiPos and _GACC.getUiPos("menu")) or UDim2.new(0,20,0,80)
        MiniBtn.Visible=false; Outer.Position=home; Outer.Visible=true
    end
    local function hideGui()
        if _GACC.saveUiPos then _GACC.saveUiPos("menu",Outer) end
        Outer.Visible=false; MiniBtn.Visible=true
    end
    ...
    CF.ElasticBehavior=Enum.ElasticBehavior.Never; CF.Active=true; CF.ZIndex=2; CF.Parent=Inner; GuiRefs.contentFrame=CF
    ...
    local function addToggleRow(parent,label,enabled,order,kbKey,onToggle)
        ...
        local TI_POP=TweenInfo.new(0.16,Enum.EasingStyle.Quint,Enum.EasingDirection.Out)
        local Track=Instance.new("Frame",Row); Track.ZIndex=3
        Track.Position=UDim2.new(1,-52,0.5,-11); Track.Size=UDim2.new(0,40,0,22)
        Track.BackgroundColor3=OFF_TRACK; Track.BackgroundTransparency=0; Track.BorderSizePixel=0
        Instance.new("UICorner",Track).CornerRadius=UDim.new(1,0)
        local TrkStroke=Instance.new("UIStroke",Track)
        TrkStroke.Color=OFF_TRACK_STROKE; TrkStroke.Thickness=1; TrkStroke.Transparency=0.25
        local Knob=Instance.new("Frame",Track); Knob.ZIndex=4; Knob.AnchorPoint=Vector2.new(0,0.5)
        Knob.Position=UDim2.new(0,3,0.5,0); Knob.Size=UDim2.new(0,16,0,16)
        Knob.BackgroundColor3=Color3.fromRGB(235,235,245); Knob.BorderSizePixel=0
        Instance.new("UICorner",Knob).CornerRadius=UDim.new(1,0)
        local st=enabled
        local function circleVisual(on)
            Track.BackgroundColor3=on and _ACC.accent or OFF_TRACK
            TrkStroke.Color=on and _ACC.accentDark or OFF_TRACK_STROKE
            Knob.BackgroundColor3=on and Color3.fromRGB(255,255,255) or Color3.fromRGB(170,170,185)
        ...
        local function setV(on)
            ...
            TrkStroke.Color=on and _ACC.accentDark or OFF_TRACK_STROKE
            Knob.BackgroundColor3=on and Color3.fromRGB(255,255,255) or Color3.fromRGB(170,170,185)
            tw(Knob,{Position=on and UDim2.new(1,-19,0.5,0) or UDim2.new(0,3,0.5,0)},TI_POP)
        end
        local Btn=Instance.new("TextButton",Row); Btn.Size=UDim2.new(0,44,0,26); Btn.Position=UDim2.new(1,-54,0.5,-13); Btn.BackgroundTransparency=1; Btn.Text=""; Btn.ZIndex=5
...
local profileLine=Instance.new("Frame",userF)
    ...
    do
        ...
        do
            ...
            for idx,def in ipairs(defs) do
                ...
                btn.BackgroundColor3=Color3.fromRGB(24,10,44); btn.BackgroundTransparency=0.12
        ...
        local function chooseStealMode(nextMode)
            ...
            for _,c in ipairs(stealChoices) do
                ...
                c.btn.BackgroundColor3=sel and _ACC.accentBg or Color3.fromRGB(24,10,44)
        ...
        for _,c in ipairs(stealChoices) do
            ...
            c.btn.MouseLeave:Connect(function()
                tw(c.btn,{BackgroundColor3=(stealMode==c.mode) and _ACC.accentBg or Color3.fromRGB(24,10,44)},TweenInfo.new(0.16))
            end)
        end
        ...
        local OFF_TRACK=Color3.fromRGB(24,10,44); local OFF_KNOB=Color3.fromRGB(110,80,160)
        -- ── classic switch toggle ──────────────────────────────────────
        local Track=Instance.new("Frame",Row); Track.ZIndex=3
        Track.Size=UDim2.new(0,40,0,22); Track.Position=UDim2.new(1,-52,0,10)
        Track.BackgroundColor3=OFF_TRACK; Track.BackgroundTransparency=0; Track.BorderSizePixel=0
        Instance.new("UICorner",Track).CornerRadius=UDim.new(1,0)
        local TrkStroke=Instance.new("UIStroke",Track)
        TrkStroke.Color=_ACC.accentDark; TrkStroke.Thickness=1; TrkStroke.Transparency=0.25
        local Knob=Instance.new("Frame",Track); Knob.ZIndex=4; Knob.AnchorPoint=Vector2.new(0,0.5)
        Knob.Position=UDim2.new(0,3,0.5,0); Knob.Size=UDim2.new(0,16,0,16)
        Knob.BackgroundColor3=Color3.fromRGB(170,170,185); Knob.BorderSizePixel=0
        Instance.new("UICorner",Knob).CornerRadius=UDim.new(1,0)
        local function stealCircleVisual(on)
            Track.BackgroundColor3=on and _ACC.accent or OFF_TRACK
            Knob.BackgroundColor3=on and Color3.fromRGB(255,255,255) or Color3.fromRGB(170,170,185)
            tw(Knob,{Position=on and UDim2.new(1,-19,0.5,0) or UDim2.new(0,3,0.5,0)},TweenInfo.new(0.16,Enum.EasingStyle.Quint,Enum.EasingDirection.Out))
        end
        ...
        ToggleBtn.Size=UDim2.new(0,44,0,26); ToggleBtn.Position=UDim2.new(1,-54,0,8)
    ...
    do
        local sBg=Instance.new("ImageLabel")
        sBg.Name="SettingsBackdrop"
        sBg.Position=CF.Position; sBg.Size=CF.Size
        sBg.BackgroundTransparency=1; sBg.BorderSizePixel=0
        sBg.Image="rbxassetid://134483767427093"
        sBg.ImageTransparency=0
        sBg.ScaleType=Enum.ScaleType.Crop
        sBg.ZIndex=1
        sBg.Visible=(CategoryRefs.active=="Settings")
        sBg.Parent=Inner
        guiCorner(sBg,10)
        task.spawn(function()
            while sBg and sBg.Parent do
                sBg.Visible=(CategoryRefs.active=="Settings")
                task.wait(0.12)
            end
        end)
    ...
    task.defer(function()
        local sp=CategoryRefs.contents["Settings"]
        if not sp then return end
        for _,object in ipairs(sp:GetDescendants()) do
