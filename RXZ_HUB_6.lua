...
local RXZ_LIB do
    ...
    local function stroke(p, t, col)
        ...
        u.Color = col or C.White
        u.Thickness = 1
        u.Transparency = t or 0.45
        u.Parent = p
        return u
    end
    local function label(parent, name, text, size, pos, ts, col, font)
    ...
    -- root gui ------------------------------------------------------------
    local old = PlayerGui:FindFirstChild("RXZ_HUB")
    ...
    local WIN_W, WIN_H = 262, 296

    local Main = Instance.new("Frame")
    Main.Name = "Main"
    Main.Size = UDim2.fromOffset(WIN_W, WIN_H)
    Main.Position = UDim2.new(0, 16, 0.5, -WIN_H / 2)
    Main.Visible = false
    ...
    -- background image (top layer over the black frame)
    local BG = Instance.new("ImageLabel")
    BG.Name = "Background"
    BG.Size = UDim2.fromScale(1, 1)
    BG.BackgroundTransparency = 1
    BG.Image = "rbxassetid://129700697019613"
    ...
    label(Header, "Title", "RXZ HUB", UDim2.new(1, -80, 0, 20), UDim2.fromOffset(12, 6), 17, C.White, Enum.Font.Bangers)
    label(Header, "Sub", "MM2 SCRIPT", UDim2.new(1, -80, 0, 12), UDim2.fromOffset(12, 25), 9, C.Gray, Enum.Font.Bangers)
    ...
    Body.Size = UDim2.new(1, -80, 1, -58)
    Body.Position = UDim2.fromOffset(74, 50)
    ...
    local function selectTab(name)
        ...
        for n, btn in pairs(tabButtons) do
            local on = n == name
            btn.BackgroundTransparency = on and 0.18 or 0.72
            btn.TextColor3 = on and C.White or C.Gray
            btn:FindFirstChildOfClass("UIStroke").Transparency = on and 0.05 or 0.6
        end
    ...
    local function newRow(parent, height)
        ...
        f.Size = UDim2.new(1, -4, 0, height or 32)
    ...
    local function notify(_, opts)
        opts = type(opts) == "table" and opts or { Title = tostring(opts) }
        local holder = Gui:FindFirstChild("Notifs")
        if not holder then
            holder = Instance.new("Frame")
            holder.Name = "Notifs"
            holder.Size = UDim2.new(0, 240, 1, -40)
            holder.Position = UDim2.new(1, -252, 0, 20)
        ...
        corner(f, 8)
        stroke(f, 0.35)
        label(f, "T", tostring(opts.Title or "RXZ HUB"), UDim2.new(1, -16, 0, 18), UDim2.fromOffset(10, 6), 13, C.White, Enum.Font.GothamBold)
        label(f, "D", tostring(opts.Description or ""), UDim2.new(1, -16, 0, 18), UDim2.fromOffset(10, 24), 11, C.Gray)
        task.delay(tonumber(opts.Time) or 3, function()
    ...
    local function makeGroupbox(page, title)
        ...
        local head = label(holder, "Head", string.upper(title or ""), UDim2.new(1, -6, 0, 16), nil, 11, C.White, Enum.Font.GothamBlack)
        head.ZIndex = 103

        return setmetatable({ _frame = holder }, Groupbox)
    end

    function Groupbox:AddToggle(idx, opts)
        opts = opts or {}
        local row = newRow(self._frame, 32)
        label(row, "L", opts.Text or idx, UDim2.new(1, -60, 1, 0), UDim2.fromOffset(10, 0), 12, C.White).ZIndex = 104

        local box = Instance.new("Frame")
        box.Size = UDim2.fromOffset(34, 18)
        ...
        local obj = { Value = opts.Default and true or false, Type = "Toggle", _callbacks = {} }

        function obj:Render()
            TweenService:Create(knob, TweenInfo.new(0.12), {
                Position = self.Value and UDim2.fromOffset(18, 2) or UDim2.fromOffset(2, 2),
                BackgroundColor3 = self.Value and C.On or C.Gray,
            }):Play()
        end
        function obj:SetValue(v, skip)
            self.Value = v and true or false
            self:Render()
            if not skip then
                if opts.Callback then pcall(opts.Callback, self.Value) end
                for _, cb in ipairs(self._callbacks) do pcall(cb, self.Value) end
            end
        end
        ...
        function obj:SetText(t) row:FindFirstChild("L").Text = t end
        function obj:AddKeyPicker(id, o) return lib:_keypicker(id, o, self) end
        function obj:Toggle() self:SetValue(not self.Value) end
        obj:Render()
    ...
    function Groupbox:AddButton(a, b)
        local text, fn
        if type(a) == "table" then text, fn = a.Text, a.Func else text, fn = a, b end
        local row = newRow(self._frame, 30)
        local l = label(row, "L", text or "Button", UDim2.new(1, -12, 1, 0), UDim2.fromOffset(0, 0), 12, C.White, Enum.Font.GothamBold)
        ...
        function obj:AddButton(x, y) return Groupbox.AddButton(self and { _frame = row.Parent } or nil, x, y) end
        return obj
    end

    function Groupbox:AddLabel(text, doesWrap)
        local row = newRow(self._frame, 26)
        row.BackgroundTransparency = 0.6
        local l = label(row, "L", text or "", UDim2.new(1, -20, 1, 0), UDim2.fromOffset(10, 0), 11, C.Gray)
        l.TextWrapped = doesWrap and true or false
        l.ZIndex = 104
        local obj = { Type = "Label" }
        function obj:SetText(t) l.Text = t end
        function obj:AddKeyPicker(id, o) return lib:_keypicker(id, o, obj, row) end
        return obj
    end
    ...
    function Groupbox:AddSlider(idx, opts)
        ...
        local row = newRow(self._frame, 42)
        local l = label(row, "L", opts.Text or idx, UDim2.new(1, -70, 0, 18), UDim2.fromOffset(10, 3), 12, C.White)
        l.ZIndex = 104
        local val = label(row, "V", "", UDim2.fromOffset(56, 18), UDim2.new(1, -66, 0, 3), 11, C.White)
        ...
        local obj = { Value = tonumber(opts.Default) or min, Type = "Slider", Min = min, Max = max, _callbacks = {} }

        local function round(v)
            local r = opts.Rounding or 0
            local m = 10 ^ r
            return math.floor(v * m + 0.5) / m
        end
        function obj:Render()
            fill.Size = UDim2.fromScale(math.clamp((self.Value - min) / math.max(max - min, 1e-6), 0, 1), 1)
            val.Text = tostring(self.Value) .. (opts.Suffix or "")
        end
        function obj:SetValue(v, skip)
            self.Value = round(math.clamp(tonumber(v) or min, min, max))
            self:Render()
            if not skip then
                if opts.Callback then pcall(opts.Callback, self.Value) end
                for _, cb in ipairs(self._callbacks) do pcall(cb, self.Value) end
        ...
        function obj:SetMin(v) min = v; self:Render() end
        function obj:SetMax(v) max = v; self:Render() end
        function obj:SetText(t) l.Text = t end
        obj:Render()
    ...
    function Groupbox:AddDropdown(idx, opts)
        ...
        local row = newRow(self._frame, 32)
        local l = label(row, "L", opts.Text or idx, UDim2.new(0.5, -8, 1, 0), UDim2.fromOffset(10, 0), 12, C.White)
        l.ZIndex = 104
        local val = label(row, "V", "---", UDim2.new(0.5, -24, 1, 0), UDim2.new(0.5, 0, 0, 0), 11, C.Gray)
        val.TextXAlignment = Enum.TextXAlignment.Right
        val.ZIndex = 104

        local listHolder = Instance.new("ScrollingFrame")
        listHolder.Size = UDim2.new(1, -4, 0, 0)
        listHolder.BackgroundColor3 = C.Row
        ...
        listHolder.ZIndex = 108
        listHolder.Parent = self._frame
        corner(listHolder, 8)
        stroke(listHolder, 0.4)
        local ll = Instance.new("UIListLayout")
        ll.Parent = listHolder

        local obj = { Value = multi and {} or nil, Values = values, Type = "Dropdown", _callbacks = {} }

        local function display()
            if multi then
                local t = {}
                for k, v in pairs(obj.Value) do if v then table.insert(t, k) end end
                val.Text = #t > 0 and table.concat(t, ", ") or "---"
            else
                val.Text = obj.Value and tostring(obj.Value) or "---"
            end
        end
        function obj:SetValue(v, skip)
            self.Value = v
        ...
        if opts.Default and type(opts.Default) ~= "number" then
            obj:SetValue(opts.Default, true)
        elseif type(opts.Default) == "number" and values[opts.Default] then
            obj:SetValue(values[opts.Default], true)
        end
        display()
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            listHolder.Size = listHolder.Visible and UDim2.new(1, -4, 0, 104) or UDim2.new(1, -4, 0, 0)
        end)
    ...
    function Groupbox:AddInput(idx, opts)
        opts = opts or {}
        local row = newRow(self._frame, 32)
        label(row, "L", opts.Text or idx, UDim2.new(0.45, 0, 1, 0), UDim2.fromOffset(10, 0), 12, C.White).ZIndex = 104
        local tb = Instance.new("TextBox")
        tb.Size = UDim2.new(0.5, -14, 0, 22)
        tb.Position = UDim2.new(0.5, 0, 0.5, -11)
        tb.BackgroundTransparency = 0.4
        tb.BackgroundColor3 = C.Row
        ...
        tb.Parent = row
        corner(tb, 6)
        local obj = { Value = tb.Text, Type = "Input", _callbacks = {} }
        function obj:SetValue(v)
            tb.Text = tostring(v)
            self.Value = tb.Text
            for _, cb in ipairs(self._callbacks) do pcall(cb, self.Value) end
        end
        function obj:OnChanged(cb) table.insert(self._callbacks, cb) return self end
        tb.FocusLost:Connect(function()
            obj.Value = tb.Text
            if opts.Callback then pcall(opts.Callback, tb.Text) end
            for _, cb in ipairs(obj._callbacks) do pcall(cb, tb.Text) end
        end)
        lib.Options[idx] = obj
        return obj
    end
    ...
    KeybindFrame.Size = UDim2.fromOffset(160, 120)
    KeybindFrame.Position = UDim2.fromOffset(16, 16)
    KeybindFrame.BackgroundColor3 = C.BG
    KeybindFrame.BackgroundTransparency = 0.15
    KeybindFrame.BorderSizePixel = 0
    KeybindFrame.Visible = false
    KeybindFrame.Parent = Gui
    corner(KeybindFrame, 10)
    stroke(KeybindFrame, 0.35)
    local kbl = Instance.new("UIListLayout")
    kbl.Padding = UDim.new(0, 2)
    kbl.Parent = KeybindFrame
    lib.KeybindFrame = KeybindFrame

    function lib:_keypicker(id, opts, parentObj, row)
        opts = opts or {}
        local obj = { Value = opts.Default or "None", Mode = opts.Mode or "Toggle", Type = "KeyPicker", _callbacks = {} }
        function obj:OnChanged(cb) table.insert(self._callbacks, cb) return self end
        function obj:OnClick(cb) self._click = cb return self end
        function obj:GetState() return self._state and true or false end
        function obj:SetValue(v)
            self.Value = type(v) == "table" and v[1] or v
            if self._row then self._row.Text = (opts.Text or id) .. " : " .. tostring(self.Value) end
        end
        local kr = Instance.new("TextLabel")
        kr.Size = UDim2.new(1, -8, 0, 18)
        kr.BackgroundTransparency = 1
        kr.TextColor3 = C.White
        kr.TextSize = 11
        kr.Font = Enum.Font.GothamSemibold
        kr.Text = (opts.Text or id) .. " : " .. tostring(obj.Value)
        kr.Parent = KeybindFrame
        obj._row = kr
        keyRows[id] = obj

        UIS.InputBegan:Connect(function(input, gpe)
            if gpe then return end
            if input.KeyCode == Enum.KeyCode[tostring(obj.Value)] then
                obj._state = true
                if obj._click then pcall(obj._click) end
                for _, cb in ipairs(obj._callbacks) do pcall(cb, true) end
                if opts.Callback then pcall(opts.Callback, true) end
                if opts.SyncToggleState and parentObj and parentObj.Toggle then parentObj:Toggle() end
            end
        end)
    ...
    function lib:CreateWindow(opts)
        ...
        function window:AddTab(name)
            ...
            local tab = {}
            function tab:AddLeftGroupbox(t) return makeGroupbox(page, t) end
            function tab:AddRightGroupbox(t) return makeGroupbox(page, t) end
            function tab:AddLeftTabbox() return { AddTab = function(_, t) return makeGroupbox(page, t) end } end
            function tab:AddRightTabbox() return { AddTab = function(_, t) return makeGroupbox(page, t) end } end
            return tab
        end
        function window:AddDialog() return { Open = function() end, Close = function() end } end
        function window:AddKeyTab() return window:AddTab("Keys") end
        function window:Toggle(state) lib:Toggle(state) end
        return window
    end

    -- loading screen -------------------------------------------------------
    function lib:CreateLoading(opts)
        opts = opts or {}
        local load = {}
        function load:SetMessage() end
        function load:SetDescription() end
        function load:SetTitle() end
        function load:UpdateStep() end
        function load:Destroy() end
        function load:Finish() end
        return load
    end

    function lib:AddDraggableLabel(text)
        local f = Instance.new("TextLabel")
        f.Size = UDim2.fromOffset(190, 26)
        f.Position = UDim2.new(0.5, -95, 0, 60)
        f.BackgroundColor3 = C.BG
        ...
        stroke(f, 0.4)
        local o = {}
        function o:SetText(t) f.Text = t end
        function o:SetVisible(v) f.Visible = v and true or false end
        function o:Destroy() f:Destroy() end
        return o
    end

    function lib:Toggle(state)
        if state == nil then state = not Main.Visible end
        Main.Visible = state and true or false
        if lib.MiniButton then lib.MiniButton.Visible = not Main.Visible end
    end
    function lib:SetWatermark() end
    function lib:SetWatermarkVisibility() end
    function lib:OnUnload(cb) lib._unload = cb end
    function lib:Unload() Gui:Destroy() end

    -- open / close button (mobile friendly)
    local OpenBtn = Instance.new("TextButton")
    OpenBtn.Size = UDim2.fromOffset(74, 30)
    OpenBtn.Position = UDim2.new(0, 16, 0, 12)
    ...
    -- minimized square (draggable, tap to open) ---------------------------
    local Mini = Instance.new("TextButton")
    Mini.Name = "Mini"
    Mini.Size = UDim2.fromOffset(52, 52)
    Mini.Position = UDim2.new(0, 16, 0, 90)
    ...
    lib.SetOpen = function(_, st) setOpen(st) end
    ...
    do
        ...
        MinBtn.Position = UDim2.new(1, -36, 0, 10)
    ...
    UIS.InputBegan:Connect(function(input, gpe)
        if gpe then return end
        local key = lib.ToggleKeybind and lib.ToggleKeybind.Value
        if key and input.KeyCode == Enum.KeyCode[tostring(key)] then
...
do
    ...
    do
        ...
        do
            ...
            function savePosition(p48, p49)
                task.spawn(function()
                    pcall(function()
                        if not isfolder("RXZHUB/configs") then
                            makefolder("RXZHUB/configs")
                        end

                        local t24 = {}

                        if isfile("RXZHUB/configs/positions.json") then
                            local ok, result = pcall(function()
                                local hs = t13.hs
                                local t25 = { readfile("RXZHUB/configs/positions.json") }
                        ...
                        writefile("RXZHUB/configs/positions.json", t13.hs:JSONEncode(t24))
                    end)
                end)
            end
            function loadPosition(p50, p51)
                local ok, result = pcall(function()
                    if not isfile("RXZHUB/configs/positions.json") then
                        return nil
                    end

                    local hs = t13.hs
                    local t26 = { readfile("RXZHUB/configs/positions.json") }
            ...
            function saveLockConfig()
                pcall(function()
                    ...
                    writefile("RXZHUB/configs/locks.json", t13.hs:JSONEncode(t37))
                end)
            end
            function loadLockConfig()
                pcall(function()
                    if not isfile("RXZHUB/configs/locks.json") then
                        return
                    end

                    local data = t13.hs:JSONDecode(readfile("RXZHUB/configs/locks.json"))
            ...
            function createDraggableButton(p72, p73, p74)
                ...
                TextButton.Size = UDim2.new(0, 68, 0, 58)
                local uDim2 = UDim2.new(0.5, math.floor(p73 * 1.85), 0.75, 0)
                local v631 = p74
                if p74 then
        ...
        function savePredictionConfig()
            task.spawn(function()
                ...
                writefile("RXZHUB/configs/prediction.json", t13.hs:JSONEncode(t41))
            end)
        end
        function loadPredictionConfig(p94, p95, p96, p97)
            if not isfile("RXZHUB/configs/prediction.json") then
                return
            end

            local ok, result = pcall(function()
                local hs = t13.hs
                local t42 = { readfile("RXZHUB/configs/prediction.json") }
        ...
        t1[31] = function()
            t13.SaveManager = RXZ_STUB_MANAGER
            t13.SaveManager:SetLibrary(t13.lib)
            t13.SaveManager:IgnoreThemeSettings()
            t13.SaveManager:SetFolder("RXZHUB/configs")
        ...
        function sendAutoFarmWebhook(p112, p113, p114)
            ...
            pcall(function()
                ...
                local t55 = {
					text = "RXZHUB • https://RXZHUB.filho.wtf"
				}
                local v1396 = os.date("!%Y-%m-%dT%H:%M:%SZ")
                local t56 = {
					title = v1376,
					url = "https://RXZHUB.filho.wtf",
					description = v1382,
					color = v1383,
					author = t49,
					thumbnail = t50,
        ...
        t1[33] = t13
        t1[34] = "version"
        t1[36] = fetch("https://RXZHUB.filho.wtf/version.lua", "v0.0.5")
        t1[33][t1[34]] = t1[36]
        t1[36] = function()
            local v956 = t13
            local lib = t13.lib
            ...
            local v961 = _UDim2.fromOffset(330, 372)
        ...
        t1[43] = function()
            setclipboard("https://discord.gg/rxzhub")
        end
    ...
    do
        ...
        local v58 = t13.window:AddTab("Settings", "settings")
    ...
    do
        ...
        local uDim2 = UDim2.new(0, 68, 0, 58)
...
local ok, result = pcall(function()
    t13.ThemeManager = RXZ_STUB_MANAGER
    t13.ThemeManager:SetLibrary(t13.lib)
    t13.ThemeManager:SetFolder("RXZHUB")
